...
const Statbox = (props) => {
    ...
    const makeStatDisplay = () => {
        let bars = [<GridItem key={`header-primary`} colSpan={2} onClick={() => handlePrimaryFilter()}><HStack spacing="6px"><Text>{props.type == "player" ? "Player" : "Hero"}</Text><Center><Text fontWeight={"bold"}>{primaryFilter}</Text></Center></HStack></GridItem>,
        <GridItem key={`header-matches`} colSpan={3} onClick={() => handleMatchesFilter()}><HStack spacing="6px"><Text>Matches</Text><Center><Text fontWeight={"bold"}>{matchesFilter}</Text></Center></HStack></GridItem>,
        <GridItem key={`header-winrate`} colSpan={3} onClick={() => handleWinRateFilter()}><HStack spacing="6px"><Text>Win Rate</Text><Center><Text fontWeight={"bold"}>{winRateFilter}</Text></Center></HStack></GridItem>,
        <GridItem key={`header-xi`} colSpan={3} onClick={() => handleXIFilter()}><HStack spacing="6px"><Text>xImp</Text><Center><Text fontWeight={"bold"}>{xIFilter}</Text></Center></HStack></GridItem>]

        let max = stats.reduce((maxMatches, currentUser) => Math.max(maxMatches, currentUser.wins + currentUser.losses), 0);

        if (props.type == "player") {
            stats.map((stat, i) => {
                bars.push(<GridItem key={`${i}-primary`} colSpan={2} display="flex" alignItems="center"><Box><Link href={"/player/" + stat.id} passHref legacyBehavior><a rel="noopener noreferrer"><Text _hover={{ color: "#808080", transition: "0.3s", }} size="md">{stat.username}</Text></a></Link></Box></GridItem>)
                bars.push(<GridItem key={`${i}-matches`} colSpan={3}><Text>{stat.wins + stat.losses}</Text><Box pt={1}>{makeBar((stat.wins + stat.losses) / max * 100, "#319795")}</Box></GridItem >)
                let winRate = (Math.round((stat.wins / (stat.wins + stat.losses) * 100) * 100) / 100).toFixed(2)
                if (isNaN(winRate)) winRate = 0
                bars.push(<GridItem key={`${i}-winrate`} colSpan={3}><Text>{isNaN(winRate) ? 0 : winRate}%</Text><Box pt={1}>{makeBar(winRate, getColour(winRate))}</Box></GridItem>)
                bars.push(<GridItem key={`${i}-xi`} colSpan={3}><Text>{stat.impact !== null ? calcImpact(stat.impact) : "No Matches"}</Text><Tooltip label={`xImp: ${Math.floor(stat.impact)}`}><Box pt={1}>{makeBar(stat.impact / 120 * 100, "#7c3aed")}</Box></Tooltip></GridItem>)
            })

        } else {
            stats.map((stat, i) => {
                console.log(stat)
                console.log(max, stat.wins + stat.losses)
                console.log((stat.wins + stat.losses) / max)
                bars.push(<GridItem key={`${i}-primary`} colSpan={2} display="flex" alignItems="center"><Image alt={stat.name} loader={() => stat.img} src={stat.img} width={60} height={90} style={{ height: "auto", objectFit: "contain", position: "relative" }} unoptimized /><Text _hover={{ cursor: "default" }} position={"absolute"} fontSize="xs" color={"rgba(0, 0, 0, 0)"}>{stat.name}</Text></GridItem>)
                bars.push(<GridItem key={`${i}-matches`} colSpan={3}><Text>{stat.wins + stat.losses}</Text><Box pt={1}>{makeBar((stat.wins + stat.losses) / max * 100, "#319795")}</Box></GridItem >)
                let winRate = (Math.round((stat.wins / (stat.wins + stat.losses) * 100) * 100) / 100).toFixed(2)
                bars.push(<GridItem key={`${i}-winrate`} colSpan={3}><Text>{winRate}%</Text><Box pt={1}>{makeBar(winRate, getColour(winRate))}</Box></GridItem>)
                bars.push(<GridItem key={`${i}-xi`} colSpan={3}><Text>{stat.impact !== null ? calcImpact(stat.impact) : "No Matches"}</Text><Box pt={1}>{makeBar(stat.impact / 120 * 100, "#7c3aed")}</Box></GridItem>)
            })
        }
