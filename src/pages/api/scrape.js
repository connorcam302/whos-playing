...
async function fetchUserData(id) {
    try {
        const result = await fetch(`${process.env.STEAMURL}/IDOTA2Match_570/getMatchHistory/v1?key=${process.env.STEAMKEY}&account_id=${id}&matches_requested=15`, {
            method: "GET",
        });
        return await result.json();
    } catch (err) {
...
async function fetchOpenDotaData(id) {
    try {
        const result = await fetch(`${process.env.OPENDOTAURL}/players/${id}/matches?date=3`, {
            method: "GET",
        });
        return result.json();
    } catch (err) {
