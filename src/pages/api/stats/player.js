...
export default async function handler({ query: { limit, days } }, res) {
    ...
    var matches = await supabase
        .from("matches")
        .select("start_time, match_data(match_id, player_id, winner, impact)")
    ...
    for (let index = 0; index < players.length; index++) {
        ...
        players[index].impact = allMatches.filter((match) => match.player_id === players[index].id).reduce((sum, match) => sum + match.impact, 0) / allMatches.filter((match) => match.player_id === players[index].id).length;
    }
